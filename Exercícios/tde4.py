...
resultado = verificarSoma(a, b, limite)
print(f"A soma de {a} e {b} é maior que {limite}? {resultado}")
...
def somarImposto(taxaImposto, custo):

    return custo + (custo * taxaImposto / 100)

taxa = float(input("Informe a taxa de imposto (em %): "))
custo = float(input("Informe o custo do item (antes do imposto): "))

custoFinal = somarImposto(taxa, custo)
print(f"O custo final do item, com o imposto aplicado, é: {custoFinal:.2f}")
...
qtd_digitos = quantidade_digitos(numero)
print(f"O número {numero} possui {qtd_digitos} dígito(s).")
